...
class Qcformbuilder_Forms
{
	 ...
	 * @since 1.4.2
	 */
	const PLUGIN_SLUG = 'qcformbuilder-forms';
	 ...
	 * @since 1.5.1
	 *
	 * @var Qcformbuilder_Forms_DB_Tables
	 */
	protected static $tables;

	/**
	 * Used to track if system has initialized and prevent recursion in init_wfb_internal() method.
	 *
	 * @since 1.3.5
	 *
	 * @var bool
	 ...
	 * @since 1.4.4
	 *
	 * @var Qcformbuilder_Forms_API_Load
	 */
	 ...
	 * Settings collection
	 *
	 * Access via Qcformbuilder_Forms::settings()
	 *
	 * @since 1.5.3
	 *
	 * @var Qcformbuilder_Forms_Settings
	 */
	...
	function __construct()
	{
		...
		add_filter('qcformbuilder_forms_get_form_processors', array($this, 'get_form_processors'));
		add_filter('qcformbuilder_forms_submit_redirect_complete', array($this, 'do_redirect'), 10, 4);
		add_action('qcformbuilder_forms_edit_end', array($this, 'calculations_templates'));
		add_filter('qcformbuilder_forms_render_get_field', array($this, 'auto_populate_options_field'), 10, 2);
		add_filter('qcformbuilder_forms_render_get_field', array($this, 'apply_conditional_groups'), 10, 2);
		add_filter('qcformbuilder_forms_view_field_paragraph', 'wpautop');

		//mailer magic
		add_filter('qcformbuilder_forms_get_magic_tags', array($this, 'set_magic_tags'), 1);

		// action
		add_action('qcformbuilder_forms_submit_complete', array($this, 'save_final_form'), 50);

		// find if profile is loaded
		add_action('wp_loaded', array($this, 'wfb_init_system'), 25);
		add_action('wp', array($this, 'wfb_init_preview'));

		// render shortcode
		add_shortcode('qcformbuilder_form', array($this, 'shortcode_handler'));
		// modal shortcode
		add_shortcode('qcformbuilder_form_modal', array($this, 'shortcode_handler'));
		add_action('wp_footer', array('Qcformbuilder_Forms_Render_Modals', 'render_footer_modals'));

		//filter shortcode atts for defaults
		add_filter('shortcode_atts_qcformbuilder_form', array('Qcformbuilder_Forms_Shortcode_Atts', 'allow_default_set'), 5, 4);
		add_filter('shortcode_atts_qcformbuilder_form_modal', array('Qcformbuilder_Forms_Shortcode_Atts', 'allow_default_set'), 5,
			4);
		add_filter('shortcode_atts_qcformbuilder_form', array('Qcformbuilder_Forms_Shortcode_Atts', 'maybe_allow_revision'), 5, 4);
		add_filter('shortcode_atts_qcformbuilder_form_modal', array('Qcformbuilder_Forms_Shortcode_Atts', 'maybe_allow_revision'),
			5, 4);

		//email settings
		add_action('qcformbuilder_forms_core_init', array('Qcformbuilder_Forms_Email_Settings', 'maybe_add_hooks'));
		add_filter('pre_update_option__qcformbuilder_forms_email_api_settings', array(
			'Qcformbuilder_Forms_Email_Settings',
			'sanitize_save'
		));

		if (current_user_can(Qcformbuilder_Forms::get_manage_cap('admin'))) {
			add_action('wp_ajax_wfb_email_save', array('Qcformbuilder_Forms_Email_Settings', 'save'));
		}

		// Add cache plugin support
		if (method_exists('LiteSpeed_Cache_API', 'esi_enabled') && LiteSpeed_Cache_API::esi_enabled()) {
			LiteSpeed_Cache_API::hook_tpl_esi('qcformbuilder_forms', 'Qcformbuilder_Forms_Render_Nonce::hook_esi');
		}

		//auto-population via Easy Pods/ Easy Queries
		add_action('qcformbuilder_forms_render_start', array(__CLASS__, 'easy_pods_queries_setup'));

		//delete file uploads that are not going in media library
		add_action('qcformbuilder_forms_submit_complete', array('Qcformbuilder_Forms_Files', 'cleanup'));
		add_action(Qcformbuilder_Forms_Files::CRON_ACTION, array('Qcformbuilder_Forms_Files', 'cleanup_via_cron'));

		//entry permission
		add_filter('qcformbuilder_forms_manage_cap', array('Qcformbuilder_Forms_Entry_UI', 'permissions_filter'), 9, 3);

		add_filter('qcformbuilder_forms_send_email', array(Qcformbuilder_Forms_Admin::get_instance(), 'block_email_on_edit'), 9);

		if (current_user_can(Qcformbuilder_Forms::get_manage_cap('admin'))) {
			$id = null;

			$view = false;
			if (isset($_GET['wfb-email-preview'], $_GET['wfb-email-preview-form'])) {
				if (wp_verify_nonce($_GET['wfb-email-preview'], $_GET['wfb-email-preview-form'])) {
					$id = $_GET['wfb-email-preview-form'];
					$view = true;
				}
			}

			new Qcformbuilder_Forms_Email_Previews($id, $view);
		...
		add_filter('qcformbuilder_forms_mailer', array('Qcformbuilder_Forms_Email_Filters', 'mail_attachment_check'), 10, 3);
		//format email - using old method in case anyone removed the hook
		add_filter('qcformbuilder_forms_mailer', array($this, 'format_message'));
		//format autoresponder email
		add_filter('qcformbuilder_forms_autoresponse_mail',
			array('Qcformbuilder_Forms_Email_Filters', 'format_autoresponse_message'));
		add_filter('qcformbuilder_forms_mailer', array('Qcformbuilder_Forms_Email_Filters', 'prepare_headers'));
		add_filter('qcformbuilder_forms_autoresponse_mail', array('Qcformbuilder_Forms_Email_Filters', 'prepare_headers'));

		/** Entry Viewer v1 */
		add_action('wp_ajax_browse_entries', array(Qcformbuilder_Forms_Entry_UI::get_instance(), 'view_entries'));
		add_action('wp_ajax_get_entry', array(Qcformbuilder_Forms_Entry_UI::get_instance(), 'view_entry'));
		add_action('qcformbuilder_forms_entry_actions', array(Qcformbuilder_Forms_Entry_UI::get_instance(), 'get_entry_actions'),
			1);

		add_action('rest_api_init', array(__CLASS__, 'init_rest_api'));

		//entry viewer shortcode
		add_shortcode(Qcformbuilder_Forms_Entry_Shortcode::get_shortcode_name(),
			array('Qcformbuilder_Forms_Entry_Shortcode', 'shortcode_callback'));

		//init Credit card # hash class here, not on hook, so it can't be unhooked
		new Qcformbuilder_Forms_Field_Credit;

		/** Load magic tag system */
		new Qcformbuilder_Forms_Magic();

		//init utm tag field handler
		$utm = new Qcformbuilder_Forms_Field_Utm();
		$utm->add_hooks();

		//clear syncer cache on form update
		add_action('qcformbuilder_forms_save_form', array('Qcformbuilder_Forms_Sync_Factory', 'clear_cache'));

		//initialize settings
		Qcformbuilder_Forms_Settings_Init::load();

		//CRON callback for deleting our fake transients
		add_action(Qcformbuilder_Forms_Transient::CRON_ACTION, array('Qcformbuilder_Forms_Transient', 'cron_callback'));
		add_action('qcformbuilder_forms_submit_complete', array('Qcformbuilder_Forms_Transient', 'cron_callback'));

		//Init GDPR exporters/erasers
        add_action( 'init', [ Qcformbuilder_Forms_GDPR::class, 'register_gdpr' ] );

		/**
		 * Runs after Qcformbuilder Forms core is initialized
		 *
		 * @since 1.3.5.3
		 */
		do_action('qcformbuilder_forms_core_init');

		/** Adding anything to this constructor after qcformbuilder_forms_core_init action is a violation of intergalactic law */
	...
	{
		return Qcformbuilder_Forms_Forms::get_form($id_name);
	}
	...
	{
		return Qcformbuilder_Forms_Forms::get_forms(true, $internal);
	...
	{
		/**
		 * Use this filter to force Qcformbuilder Forms styles to enqueue early -- IE force them into header
		 *
		 * @since 1.5.0
		 *
		 * @param bool $use To use or not
		 */
		$use = apply_filters('qcformbuilder_forms_force_enqueue_styles_early', false);
		$on_page = self::check_for_forms_on_page();
		if ($use || $on_page) {
			add_action('wp_enqueue_scripts', array('Qcformbuilder_Forms_Render_Assets', 'optional_style_includes'));
	...
	public static function check_for_forms_on_page($post = null)
	{
		...
		$has_shortcode = has_shortcode($post->post_content, 'qcformbuilder_form');
		if ($has_shortcode) {
			return true;
		}

		$has_shortcode = has_shortcode($post->post_content, 'qcformbuilder_form_modal');
		if ($has_shortcode) {
			return true;
		}
		...
		if (is_array($sidebars) && !empty($sidebars)) {
			$form_widgets = get_option('widget_qcformbuilder_forms_widget');
			unset($sidebars['wp_inactive_widgets']);
			foreach ($sidebars as $sidebar => $set) {
				if (is_active_sidebar($sidebar)) {
					foreach ($set as $setup) {
						if (false !== strpos($setup, 'qcformbuilder_forms_widget-')) {
							$widget_instance = str_replace('qcformbuilder_forms_widget-', '', $setup);
	...
	{
		_deprecated_function(__METHOD__, '1.5.0.7', 'Qcformbuilder_Forms_Render_Modals::modal_button');
		return Qcformbuilder_Forms_Render_Modals::modal_button($atts, $content, $form, $modal_id);
	}
	...
	public static function load_field($form, $field_base_id)
	{
		if (!empty($form['fields'][$field_base_id])) {
			...
			//probably bad, but opportunity to defined using "qcformbuilder_forms_render_setup_field"
			$field = array();
		}
		/**
		 ...
		 */
		$field = apply_filters('qcformbuilder_forms_render_setup_field', $field, $form);
	 ...
	 *
	 */
	public static function init_wfb_internal()
	{

		if (false == self::$internal_init) {
			add_rewrite_tag('%wfb_api%', '([^&]+)');
			add_rewrite_tag('%wfb_entry%', '([^&]+)');
			// INIT API
			add_rewrite_rule('^wfb-api/([^/]*)/([^/]*)/?', 'index.php?wfb_api=$matches[1]&wfb_entry=$matches[2]', 'top');
			add_rewrite_rule('^wfb-api/([^/]*)/?', 'index.php?wfb_api=$matches[1]', 'top');

			self::$internal_init = true;


			// check update version
			$db_version = get_option('WFB_DB', 0);
			update_option( 'WFB_DB', 7 );
			$force_update = false;

			// ensure that admin can only force update
			if (Qcformbuilder_Forms::get_manage_cap('admin') && isset($_GET['cal_db_update'])) {
				$force_update = (bool)wp_verify_nonce($_GET['cal_db_update']);
			}

			include_once WFBCORE_PATH . 'includes/updater.php';

			if ( WFB_DB > $db_version || $force_update) {
				self::check_tables();
				if ($db_version < 2 || $force_update) {
					qcformbuilder_forms_db_v2_update();
				}

				if ( $db_version < 4 || $force_update) {
					self::activate_qcformbuilder_forms(true);
					qcformbuilder_forms_write_db_flag(4);
				}

				if (($db_version < 6 || $force_update) && class_exists('Qcformbuilder_Forms_Forms')) {
					qcformbuilder_forms_db_v6_update();
				}

                if (($db_version < 7 || $force_update) ) {
                    qcformbuilder_forms_db_v7_update();
                }

				if (($db_version < 8 || $force_update) ) {
					Qcformbuilder_Forms_Transient::clear_wpcron();
				}

				qcformbuilder_forms_write_db_flag(WFB_DB );

			} else {
				$version = qcformbuilder_forms_get_last_update_version();
				if (empty($version) || version_compare($version, WFBCORE_VER) !== 0) {
					update_option('_qcformbuilderforms_lastupdate', WFBCORE_VER);
				}
	 ...
	 */
	public static function activate_qcformbuilder_forms($force = false)
	{

		//if running on activation hook, load order is wrong.
		if( ! did_action( 'qcformbuilder_forms_includes_complete' ) ){
			qcformbuilder_forms_load();
		}
		...
		$columns = $wpdb->get_results("SHOW COLUMNS FROM `" . $wpdb->prefix . "wfb_form_entry_values`", ARRAY_A);  //db call ok; no-cache ok  
		...
		if (!in_array('field_id', $fields)) {
			...
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wfb_form_entry_values` ADD `field_id` varchar(20) NOT NULL AFTER `entry_id`;");  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange

			$wpdb->query("CREATE INDEX `field_id` ON `" . $wpdb->prefix . "wfb_form_entry_values` (`field_id`); ");  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange

			// update all entries
			$forms = $wpdb->get_results("SELECT `id`,`form_id` FROM `" . $wpdb->prefix . "wfb_form_entries`", ARRAY_A);  //db call ok; no-cache ok 

			$known = array();
			if (!empty($forms)) {
				foreach ($forms as $form) {
					if (!isset($known[$form['form_id']])) {
						$config = Qcformbuilder_Forms_Forms::get_form($form['form_id']);
						if (empty($config)) {
							continue;
						}
					...
					foreach ($config['fields'] as $field_id => $field) {
						...
						$wpdb->update($wpdb->prefix . "wfb_form_entry_values", array('field_id' => $field_id), array(
							'entry_id' => $form['id'],
							'slug' => $field['slug']
		...
		// add status
		$columns = $wpdb->get_results("SHOW COLUMNS FROM `" . $wpdb->prefix . "wfb_form_entries`", ARRAY_A);  //db call ok; no-cache ok 
		...
		if (!in_array('status', $fields) && $version < '1.2.0') {
			...
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wfb_form_entries` ADD `status` varchar(20) NOT NULL DEFAULT 'active' AFTER `datestamp`;");  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange

			$wpdb->query("CREATE INDEX `status` ON `" . $wpdb->prefix . "wfb_form_entries` (`status`); ");  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.SchemaChange
	...
	{
		if (!did_action('qcformbuilder_forms_checked_tables')) {
			global $wpdb;
			if (!class_exists('Qcformbuilder_Forms_DB_Tables')) {
				include_once WFBCORE_PATH . 'classes/db/tables.php';
			}

			self::$tables = new Qcformbuilder_Forms_DB_Tables($wpdb);
			 ...
			 * @since 1.5.1
			 *
			 * @param Qcformbuilder_Forms_DB_Tables $tables Qcformbuilder_Forms_DB_Tables object
			 */
			do_action('qcformbuilder_forms_checked_tables', self::$tables);
	...
	{
		_deprecated_function(__METHOD__, '1.5.9', 'Qcformbuilder_Forms_Email_Filters::mail_attachment_check');
		return Qcformbuilder_Forms_Email_Filters::mail_attachment_check($mail, $data, $form);
	}
	...
	public static function captcha_check($value, $field, $form)
	{
		...
		if (empty($result->success)) {
			return new WP_Error('error', __("The captcha wasn't entered correctly.",
					'qcformbuilder-forms') . ' <a href="#" class="reset_' . sanitize_text_field($_POST[$field['ID']]) . '">' . __('Reset',
					'qcformbuilder-forms') . '<a>.');
		}
	...
	public static function update_field_data($field, $entry_id, $form)
	{
		...
		$not_support = Qcformbuilder_Forms_Fields::not_support($field_type, 'entry_list');
		if ($not_support) {
			return;
		}
		...
		if (has_filter('qcformbuilder_forms_update_field')) {
			$new_data = apply_filters('qcformbuilder_forms_update_field', $new_data, $field, $form);
		}

		if (has_filter('qcformbuilder_forms_update_field_' . $field['type'])) {
			$new_data = apply_filters('qcformbuilder_forms_update_field_' . $field['type'], $new_data, $field, $form);
		}

		if ($original_data !== null) {

			$wpdb->delete($wpdb->prefix . 'wfb_form_entry_values', array(
				'entry_id' => $entry_id,
				'field_id' => $field['ID']
			));  //db call ok; no-cache ok 
		...
		foreach ((array)$new_data as $entry_data) {
			...
			Qcformbuilder_Forms_Entry_Field::insert($new_entry);
		}
	...
	public static function save_field_data($field, $entry_id, $form)
	{
		...
		foreach ((array)$data as $key => $raw_entry) {
			...
			$entry = Qcformbuilder_Forms_Sanitize::sanitize($raw_entry);

			if (has_filter('qcformbuilder_forms_save_field')) {
				/**
				 ...
				 */
				$entry = apply_filters('qcformbuilder_forms_save_field', $entry, $field, $form, $entry_id);
			}

			if (has_filter('qcformbuilder_forms_save_field_' . $field['type'])) {
				/**
				 ...
				 * @param int $entry_id ID of entry to save for  @since 1.5.2
				 */
				$entry = apply_filters('qcformbuilder_forms_save_field_' . $field['type'], $entry, $field, $form, $entry_id);
			}
			...
			// Save
			Qcformbuilder_Forms_Entry_Field::insert($field_item);
		}

		if (!empty($keyed)) {

			if (has_filter('qcformbuilder_forms_save_field_combined' . $field['type'])) {
				$data = apply_filters('qcformbuilder_forms_save_field_combined' . $field['type'], $entry, $field);
			}
	...
	public static function save_final_form($form)
	{
		...
		if (isset($_POST['_wfb_frm_edt'])) {
			// is edit
			//check user can edit this item.
			$user_id = get_current_user_id();
			$details = Qcformbuilder_Forms::get_entry_detail($_POST['_wfb_frm_edt'], $form);

			// check token
			if (is_array($details) && isset($_POST['_wfb_frm_edt_tkn'])) {

				$validated = Qcformbuilder_Forms_Entry_Token::verify_token($_POST['_wfb_frm_edt_tkn'], $details['id'],
					$form['ID']);
				if (is_wp_error($validated)) {
					return $validated;
				} else {
					$entry_id = $details['id'];
					$edit_token = Qcformbuilder_Forms_Entry_Token::create_entry_token($entry_id, $form);
				}
		...
		if (!empty($form['db_support'])) {
			Qcformbuilder_Forms_Save_Final::save_in_db($form, $entryid);
		}


		if (self::should_send_mail($form, $transdata)) {
			Qcformbuilder_Forms_Save_Final::do_mailer($form, $entryid);
		}
	...
	public static function debug_mail_send($phpmailer)
	{
		...
		$wpdb->insert($wpdb->prefix . 'wfb_form_entry_meta', $meta_entry);  //db call ok;
			...
			foreach ($form['processors'] as $processor) {
				if ($processor['type'] == 'form_redirect') {
					...
					if (!empty($processor['config']['url'])) {
						...
						// set message
						add_filter('qcformbuilder_forms_render_notices', array(
							'Qcformbuilder_Forms',
						...
						if (!empty($referrer['query'])) {
							...
							if (isset($referrer['query']['wfb_su'])) {
								unset($referrer['query']['wfb_su']);
							}
	...
	public static function send_auto_response($config, $form)
	{
		...
		$config = apply_filters('qcformbuilder_forms_autoresponse_config', $config, $form);
		...
		if (Qcformbuilder_Forms_Email_Prepare::is_list($config['recipient_email'])) {
			$recipients = Qcformbuilder_Forms_Email_Prepare::prepare_email_array(explode(',', $config['recipient_email']));
			if (!qcformbuilder_forms_pro_is_active()) {
				$send_local = true;
			} else {
				$send_local = \qcformbuilderwp\qcformbuilderforms\pro\container::get_instance()->get_settings()->get_form($form['ID'])->should_send_local();
			}
			if ($send_local) {
		...
		if (!is_email($config['sender_email'])) {
			$config['sender_email'] = Qcformbuilder_Forms_Email_Fallback::get_fallback($form);
		}

		if (!empty($config['reply_to'])) {
			$email_message['replyto'] = Qcformbuilder_Forms::do_magic_tags($config['reply_to']);
		...
		if (!empty($config['cc'])) {
			$email_message['cc'] = Qcformbuilder_Forms::do_magic_tags($config['cc']);

			$cc_array = array_map('trim', preg_split('/[;,]/', Qcformbuilder_Forms::do_magic_tags($config['cc'])));
		...
		if (!empty($config['bcc'])) {
			$email_message['bcc'] = Qcformbuilder_Forms::do_magic_tags($config['bcc']);

			$bcc_array = array_map('trim', preg_split('/[;,]/', Qcformbuilder_Forms::do_magic_tags($config['bcc'])));
		 ...
		 *
		 */
		$email_message = apply_filters('qcformbuilder_forms_autoresponse_mail', $email_message, $config, $form, $entry_id);
		if (null === $email_message) {
			return;
		}

		if ('wp' !== Qcformbuilder_Forms_Email_Settings::get_method()) {
			...
			Qcformbuilder_Forms_Save_Final::do_mailer($form, null, null, $email_message);

			return;
		}

		do_action('qcformbuilder_forms_do_autoresponse', $config, $form);
		...
		if (!$sent) {
			 ...
			 */
			do_action('qcformbuilder_forms_autoresponder_failed', $email_message, $config, $form);
		}
	...
	public function get_form_processors($processors)
	{
		$internal_processors = array(
			'auto_responder' => array(
				"name" => __('Auto Responder', 'qcformbuilder-forms'),
				"description" => __('Sends out an auto response e-mail', 'qcformbuilder-forms'),
				"post_processor" => array($this, 'send_auto_response'),
				"template" => WFBCORE_PATH . "processors/auto_responder/config.php",
				"default" => array(
					'subject' => __('Thank you for contacting us', 'qcformbuilder-forms')
				),
			),
			'form_redirect' => array(
				"name" => __('Redirect', 'qcformbuilder-forms'),
				"description" => __('Redirects user to URL on successful submit', 'qcformbuilder-forms'),
				"template" => WFBCORE_PATH . "processors/redirect/config.php",
				"single" => false
			),
			'conditional_recipient' => array(
				"name" => __('Conditional Recipient', 'qcformbuilder-forms'),
				"description" => __('Send email to different recipients depending on conditions', 'qcformbuilder-forms'),
				"template" => WFBCORE_PATH . "processors/conditional_recipient/config.php",
				'post_processor' => array('Qcformbuilder_Forms_Processor_Conditional_Recipient', 'post_processor'),
				"single" => false
			),
			'increment_capture' => array(
				"name" => __('Increment Value', 'qcformbuilder-forms'),
				"description" => __('Increment a value per entry.', 'qcformbuilder-forms'),
				"processor" => array($this, 'increment_value'),
				"template" => WFBCORE_PATH . "processors/increment/config.php",
				"single" => true,
				"conditionals" => false,
				"magic_tags" => array(
					'increment_value'
		...
		if (!empty($wp_api_key)) {
			$internal_processors['akismet'] = array(
				"name" => __('Akismet', 'qcformbuilder-forms'),
				"description" => __('Anti-spam filtering', 'qcformbuilder-forms'),
				"pre_processor" => array($this, 'akismet_scanner'),
				"template" => WFBCORE_PATH . "processors/akismet/config.php",
				"single" => false,
			);
		}
	...
	public function increment_value($config, $form)
	{
		...
		if (!empty($config['field'])) {
			...
			$field_value = Qcformbuilder_Forms::get_field_data($field_id, $form);
		}
		 ...
		 * @param array $form Form config
		 */
		$increment_value = apply_filters('qcformbuilder_forms_incremental_value', $increment_value, $saved_value, $config,
			$form);
	...
	/**
	 * Catch known functions that were supported at https://quantumcloud.com/doc/calculation-fields/ and are not sipported by parser https://github.com/denissimon/formula-parser
	 *
	 * @since 1.8.5
	 * 
	...
	static public function calculation_deprecated_caught_process( $formula, $deprecated )
	{
		if( current_user_can( Qcformbuilder_Forms::get_manage_cap('admin') ) ) {
			...
			$message = sprintf(
				'"%s" %s',
				$deprecated,
				__('has been deprecated in calculation fields for compatibility with PHP 7.2 or later', 'qcformbuilder-forms')
			);
			 ...
			 */
			add_filter('qcformbuilder_forms_render_notices', function( $out ) use ( $message ){
	...
	{
		if (false === Qcformbuilder_Forms_Field_Util::check_conditional($field, $form)) {
			return;
		}
		...
		if (!empty($field['config']['manual'])) {
			...
			if (!empty($hastags[1])) {
				...
				foreach ($hastags[1] as $tag_key => $tag) {
					...
					foreach (Qcformbuilder_Forms_Forms::get_fields($form, false) as $key_id => $fcfg) {
						if ($fcfg['slug'] === $tag) {
							$binds[] = '#' . $key_id;
							$bindfields[] = '"' . $key_id . '"';
		...
		foreach (Qcformbuilder_Forms_Forms::get_fields($form, false) as $fid => $form_field) {
			if (false !== strpos($formula, $fid)) {
				$number = Qcformbuilder_Forms_Field_Calculation::get_value($form_field, $form);
		...
		if (false !== strpos($formula, '/0')) {
			return new WP_Error($field['ID'] . '-calculation',
				__('Calculation is invalid (division by zero)', 'qcformbuilder-forms'));
		}
		...
		if(version_compare(PHP_VERSION, '7.2.0', '<')){
			...
			//Check if the formula use a symbol or function not supported by the parser but declared as supported at https://quantumcloud.com/doc/calculation-fields/
			...
			if( $deprecated === false ){ 
				...
				try {
					...
					$total = self::calculation_deprecated_caught_process( $formula, __('A symbol in the formula', 'qcformbuilder_forms') );
				}
		...
		if (is_infinite($total) || !is_numeric($total)) {
			return new WP_Error($field['ID'] . '-calculation', __('Calculation is invalid', 'qcformbuilder-forms'));
		}

		$total = Qcformbuilder_Forms_Field_Util::format_calc_field($field, $total);
	...
	{
		include WFBCORE_PATH . "fields/calculation/line-templates.php";
	}
		...
		if (!empty($field['config']['auto'])) {
			...
			switch ($field['config']['auto_type']) {
				...
				case 'easy-query' :
					 ...
					 * @param array $form Form config
					 */
					$args = apply_filters('qcformbuilder_forms_autopopulate_post_type_args', $args, $field);
					 ...
					 */
					$field_for_value = apply_filters('qcformbuilder_forms_autopopulate_options_post_value_field',
					 ...
					 * @param array $posts Current post collection.
					 */
					$field_for_label = apply_filters('qcformbuilder_forms_autopopulate_options_post_label_field',
				...
				case 'taxonomy':
					 ...
					 * @param array $form Form config
					 */
					$args = apply_filters('qcformbuilder_forms_autopopulate_taxonomy_args', $args);
					 ...
					 */
					$field_for_value = apply_filters('qcformbuilder_forms_autopopulate_options_taxonomy_value_field',
					 ...
					 * @param array $posts Current term collection.
					 */
					$field_for_label = apply_filters('qcformbuilder_forms_autopopulate_options_taxonomy_label_field', 'name',
						$field, $form, $terms);
	...
	static public function check_condition($conditions, $form, $entry_id = null)
	{
		 ...
		 */
		$result = apply_filters('qcformbuilder_forms_pre_check_condition', null, $conditions, $form, $entry_id);
		 ...
		 * @param int|null $entry_id Entry ID. May be null.
		 */
		return apply_filters('qcformbuilder_forms_post_check_condition', $result, $conditions, $form, $entry_id);
	}
	...
	static public function form_redirect($type, $url, $form, $processid)
	{
		...
		$url = apply_filters('qcformbuilder_forms_redirect_url', $url, $form, $processid);
		$url = apply_filters('qcformbuilder_forms_redirect_url_' . $type, $url, $form, $processid);

		do_action('qcformbuilder_forms_redirect', $type, $url, $form, $processid);
		do_action('qcformbuilder_forms_redirect_' . $type, $url, $form, $processid);

		if (!empty($url)) {
			wfb_redirect($url, 302);
			exit;
		}
	...
	public function set_magic_tags($tags)
	{
		...
		$tags['system'] = array(
			'type' => __('System Tags', 'qcformbuilder-forms'),
			'tags' => $system_tags,
			'wrap' => array('{', '}')
		);

		// get processor tags
		$processors = Qcformbuilder_Forms_Processor_Load::get_instance()->get_processors();
		if (!empty($processors)) {
	...
	static public function do_magic_tags($value, $entry_id = null, $magic_caller = array())
	{
		...
		if (null !== $entry_id) {
			$entry_details = Qcformbuilder_Forms_Magic_Doer::magic_tag_meta_prepare($entry_id);

		}

		if (is_string($value)) {
			if (!empty($entry_details)) {
				$value = Qcformbuilder_Forms_Magic_Doer::do_processor_magic($value, $entry_details);
			}

			// check for magics
			$value = Qcformbuilder_Forms_Magic_Doer::do_bracket_magic($value, $this_form, $entry_id, $magic_caller,
				$referrer);

			// fields
			$value = Qcformbuilder_Forms_Magic_Doer::do_field_magic($value, $entry_id, $this_form);
		}
	...
	static public function get_field_types()
	{
		...
		return Qcformbuilder_Forms_Fields::get_all();
	...
	{
		if (is_string($form)) {
			$form_cfg = Qcformbuilder_Forms_Forms::get_form($form);
	...
	static public function set_field_data($field_id, $data, $form, $entry_id = false)
	{
		...
		if (is_array($field) && false === Qcformbuilder_Forms_Field_Util::check_conditional($field, $form)) {
			return;
		}
	...
	static public function get_field_data($field_id, $form, $entry_id = false, $check_conditionals = true)
	{
		...
		$field = Qcformbuilder_Forms_Field_Util::get_field($field_id, $form);
		if ($check_conditionals && is_array($field) && false === Qcformbuilder_Forms_Field_Util::check_conditional($field,
				$form)) {
			return;
		}
		...
		if (!empty($entry_id) && isset($form['fields'][$field_id])) {
			...
			if (!$entry) {
				...
				$entry = $wpdb->get_results($wpdb->prepare(
					"SELECT `value` FROM `" . $wpdb->prefix . "wfb_form_entry_values` WHERE `entry_id` = %d AND `field_id` = %s",
					$entry_id, $field_id
				), ARRAY_A);   //db call ok; no-cache ok 

				//allow plugins to alter the value
				$entry = apply_filters('qcformbuilder_forms_get_field_entry', $entry, $field_id, $form, $entry_id);
		...
		if (isset($form['fields'][$field_id])) {
			...
			// get field
			$field = apply_filters('qcformbuilder_forms_render_setup_field', $form['fields'][$field_id], $form);
			...
			// apply field filter
			if (has_filter('qcformbuilder_forms_process_field_' . $field['type'])) {
				$entry = apply_filters('qcformbuilder_forms_process_field_' . $field['type'], $entry, $field, $form);
	...
	static public function get_field_by_slug($slug, $form)
	{
		...
		return Qcformbuilder_Forms_Field_Util::get_field_by_slug($slug, $form);
	...
	static public function get_entry_detail($entry_id, $form = null)
	{
		...
		$entry = $wpdb->get_row($wpdb->prepare("SELECT * FROM `" . $wpdb->prefix . "wfb_form_entries` WHERE `id` = %d",
			$entry_id), ARRAY_A); //db call ok; no-cache ok 

		if (!empty($entry)) {
			if (null === $form) {
				$form = Qcformbuilder_Forms_Forms::get_form($entry['form_id']);
				if (empty($form)) {
					return null;
				}
			}
		...
		$entry = apply_filters('qcformbuilder_forms_get_entry_detail', $entry, $entry_id, $form);
	...
	static public function get_entry_meta($entry_id, $form, $type = null)
	{
		...
		$entry_meta_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM `" . $wpdb->prefix . "wfb_form_entry_meta` WHERE `entry_id` = %d", $entry_id), ARRAY_A);   //db call ok; no-cache ok 
		...
		if (!empty($entry_meta_data)) {
			...
			foreach ($entry_meta_data as $meta_index => $meta) {
				...
				$group = 'meta';
				$meta = apply_filters('qcformbuilder_forms_get_entry_meta', $meta, $form);
				...
				if (isset($form['processors'][$meta['process_id']]) || $meta['process_id'] == '_debug_log') {
					...
					if ($meta['process_id'] == '_debug_log') {
						...
						$entry_meta['debug'] = array(
							'name' => __('Mailer Debug', 'qcformbuilder-forms'),
							'data' => array(
								'_debug_log' => array(
									'entry' => array(
										'log' => Qcformbuilder_Forms_Sanitize::remove_scripts($meta)
									)
								)
							)
						);
					...
					$meta = apply_filters('qcformbuilder_forms_get_entry_meta_' . $form['processors'][$meta['process_id']]['type'],
					...
					if (!empty($meta)) {
						if (!isset($entry_meta[$group])) {
							...
							if (isset($form['processors'][$meta['process_id']]['type']) && isset($processors[$form['processors'][$meta['process_id']]['type']])) {
								...
								if ($meta['process_id'] == '_debug_log') {
									$meta_name = __('Mailer Debug', 'qcformbuilder-forms');
								} else {
	...
	static public function get_submission_data($form, $entry_id = false, $check_conditionals = true)
	{
		...
		if (is_string($form)) {
			...
			if (!isset($form['ID']) || $form['ID'] !== $form_id) {
				return new WP_Error('fail', __('Invalid form ID', 'qcformbuilder-forms'));
			}
		}
	...
	{
		/** You MUST not add anything before qcformbuilder_forms_submit_process_before action.  */

		/**
		 * Runs before Qcformbuilder_Forms::process_submission() does anything
		 *
		 * @since 1.5.7
		 */
		do_action('qcformbuilder_forms_submit_process_before');
		...
		if (!empty($referrer['query'])) {
			parse_str($referrer['query'], $referrer['query']);
			if (isset($referrer['query']['wfb_er'])) {
				unset($referrer['query']['wfb_er']);
			}
			if (isset($referrer['query']['wfb_su'])) {
				unset($referrer['query']['wfb_su']);
			}
		}
		if ((isset($_POST['_wfb_cr_pst']) && !is_object($post)) || (isset($_POST['_wfb_cr_pst']) && $post->ID !== (int)$_POST['_wfb_cr_pst'])) {
			$post = get_post((int)$_POST['_wfb_cr_pst']);
		}
		// get form and check
		$form = Qcformbuilder_Forms_Forms::get_form($_POST['_wfb_frm_id']);
		if (empty($form['ID']) || $form['ID'] != $_POST['_wfb_frm_id']) {
			return;
		}

		// instance number
		$form_instance_number = 1;
		if (isset($_POST['_wfb_frm_ct'])) {
			$form_instance_number = $_POST['_wfb_frm_ct'];
		}

		// check honeypot
		if (Qcformbuilder_Forms_Field_Honey::active($form)) {
			$passed = Qcformbuilder_Forms_Field_Honey::check($_POST, $form);
			if (!$passed) {
				$url = Qcformbuilder_Forms_Field_Honey::redirect_url($referrer, $form_instance_number, $process_id);

				self::form_redirect('complete', $url, $form, uniqid('_wfb_bliss_'));
			}

		}

		// init filter
		$form = apply_filters('qcformbuilder_forms_submit_get_form', $form);
		 ...
		 *
		 */
		do_action('qcformbuilder_forms_submit_start', $form, $process_id);


		if (!empty($form['fields'])) {
			foreach ($form['fields'] as $field_id => $field) {
				$field = Qcformbuilder_Forms_Field_Util::get_field($field, $form, true);
		...
		if (!empty($field_types)) {
			foreach ($field_types as $fieldType => $fieldConfig) {
				...
				if (isset($fieldConfig['handler'])) {
					add_filter('qcformbuilder_forms_process_field_' . $fieldType, $fieldConfig['handler'], 10, 3);
				}
				// check for a hash
				if (isset($fieldConfig['save'])) {
					add_filter('qcformbuilder_forms_save_field_' . $fieldType, $fieldConfig['save'], 10, 4);
				}
				// check for a hash
				if (isset($fieldConfig['validate'])) {
					add_filter('qcformbuilder_forms_validate_field_' . $fieldType, $fieldConfig['validate'], 10, 3);
				}
			}
		}

		// SET process ID
		if (isset($_GET['wfb_er'])) {
			$_POST['_wfb_frm_tr'] = $_GET['wfb_er'];
		}

		if (isset($_POST['_wfb_frm_tr'])) {
			$pretransient = Qcformbuilder_Forms_Transient::get_transient($_POST['_wfb_frm_tr']);
			if (!empty($pretransient['transient']) && $pretransient['transient'] === $_POST['_wfb_frm_tr']) {
		...
		if (empty($process_id)) {
			$process_id = uniqid('_wfb_process_');
		}

		// initialize data
		$entry_id = false;
		if (isset($_POST['_wfb_frm_edt'])) {
			$entry_id = (int)$_POST['_wfb_frm_edt'];
		}
		...
		$transdata = apply_filters('qcformbuilder_forms_submit_transient_setup', $transdata);
		...
		if (isset($_POST['_wfb_frm_edt'])) {
			// is edit
			//check user can edit this item.
			$transdata['edit'] = (int)$_POST['_wfb_frm_edt'];
			// set entry_id
			self::set_field_data('_entry_id', $transdata['edit'], $form);
			$details = self::get_entry_detail($_POST['_wfb_frm_edt'], $form);
			$user_id = get_current_user_id();

			// check token
			if (isset($_POST['_wfb_frm_edt_tkn'])) {

				$validated = Qcformbuilder_Forms_Entry_Token::verify_token($_POST['_wfb_frm_edt_tkn'], $entry_id, $form['ID']);
				if (is_wp_error($validated)) {
					return $validated;
				} else {
					$edit_token = Qcformbuilder_Forms_Entry_Token::create_entry_token($entry_id, $form);
				...
				if (empty($user_id)) {
					...
					$transdata['note'] = __('Permission denied or entry does not exist.', 'qcformbuilder-forms');
				} else {
		...
		//@see https://github.com/QcformbuilderWP/Qcformbuilder-Forms/issues/1579
		if (empty($_POST['_wfb_future'])) {
			$future_fields = '';
		} else {
			$future_fields = $_POST['_wfb_future'];
		}
		...
		foreach ($form['fields'] as $field_id => $field) {
			...
			if (is_wp_error($entry)) {
				...
				$failed = false;
				// run validators
				if (has_filter('qcformbuilder_forms_validate_field_' . $field['type'])) {
					/**
					 ...
					 * @param array $form Form config
					 */
					$entry = apply_filters('qcformbuilder_forms_validate_field_' . $field['type'], $entry, $field, $form);
				}
				 ...
				 * @param array $form Form config
				 */
				$entry = apply_filters('qcformbuilder_forms_validate_field_' . $field['ID'], $entry, $field, $form);
				...
				if (!empty($field['required'])) {
					...
					if (is_wp_error($entry)) {
						...
						$transdata['fields'][$field_id] = $field['label'] . ' ' . __('is required', 'qcformbuilder-forms');
					}
				}
			}
		...
		if (!empty($transdata['fields']) || !empty($transdata['error'])) {
			...
			// set error transient
			$transdata = apply_filters('qcformbuilder_forms_submit_return_transient', $transdata, $form, $referrer,
				$process_id);
			$transdata = apply_filters('qcformbuilder_forms_submit_return_transient_required', $transdata, $form, $referrer,
				$process_id);
			...
			$referrer = apply_filters('qcformbuilder_forms_submit_return_redirect', $referrer, $form, $process_id);
			$referrer = apply_filters('qcformbuilder_forms_submit_return_redirect_required', $referrer, $form, $process_id);

			Qcformbuilder_Forms_Transient::set_transient($process_id, $transdata, $transdata['expire']);
		...
		// has processors
		do_action('qcformbuilder_forms_submit_start_processors', $form, $referrer, $process_id);
		if (!isset($form['processors'])) {
			$form['processors'] = array();
		}

		// get all form processors
		$form_processors = Qcformbuilder_Forms_Processor_Load::get_instance()->get_processors();
		 ...
		 */
		do_action('qcformbuilder_forms_submit_pre_process_start', $form, $referrer, $process_id);
		 ...
		 */
		foreach ($form['processors'] as $processor_id => $processor) {
			// the wfb_version value in the form was introduced in 1.3.2
			// so if its set, its safe to asume the runtimes are set.
			if (!isset($form['wfb_version'])) {
				// nope
			...
			if (isset($form_processors[$processor['type']])) {
				...
				if (!empty($process_line_data)) {
					if (is_array($process_line_data)) {
						...
						// set error transient
						$transdata = apply_filters('qcformbuilder_forms_submit_return_transient', $transdata, $form,
							$referrer, $process_id);
						$transdata = apply_filters('qcformbuilder_forms_submit_return_transient_pre_process', $transdata,
						...
						$referrer = $referrer['path'] . '?' . http_build_query($query_str);
						$referrer = apply_filters('qcformbuilder_forms_submit_return_redirect', $referrer, $form,
							$process_id);
						$referrer = apply_filters('qcformbuilder_forms_submit_return_redirect-' . $processor['type'],
		 ...
		 * @param string $process_id Unique ID for this processing
		 */
		do_action('qcformbuilder_forms_submit_pre_process_end', $form, $referrer, $process_id);
		...
		if (empty($transdata['edit']) && !empty($form['db_support'])) {
			...
			$wpdb->insert($wpdb->prefix . 'wfb_form_entries', $new_entry); //db call ok;
			 ...
			 */
			do_action('qcformbuilder_forms_entry_saved', $entryid, $new_entry, $form);

			// save entry_id
			self::set_field_data('_entry_id', $entryid, $form);
			$token = Qcformbuilder_Forms_Entry_Token::create_entry_token($entryid, $form);
		 ...
		 */
		do_action('qcformbuilder_forms_submit_process_start', $form, $referrer, $process_id, $entryid);
		/// PROCESS
		foreach ($form['processors'] as $processor_id => $processor) {
			if (isset($form_processors[$processor['type']])) {
				// has processor
				...
				if (!empty($transdata['error'])) {
					...
					if (!empty($entryid) && !empty($new_entry) && $new_entry['status'] == 'pending') {
						// kill it with fire
						$wpdb->delete($wpdb->prefix . 'wfb_form_entries', array('id' => $entryid));  //db call ok; no-cache ok 
					}
					// set error transient
					$transdata = apply_filters('qcformbuilder_forms_submit_error_transient', $transdata, $form, $referrer,
						$process_id);
					$transdata = apply_filters('qcformbuilder_forms_submit_error_transient_pre_process', $transdata, $form,
						$referrer, $process_id);
					...
					$referrer = $referrer['path'] . '?' . http_build_query($query_str);
					$referrer = apply_filters('qcformbuilder_forms_submit_error_redirect', $referrer, $form, $process_id);
					$referrer = apply_filters('qcformbuilder_forms_submit_error_redirect_pre_process', $referrer, $form,
						$process_id);
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('qcformbuilder_forms_submit_process_end', $form, $referrer, $process_id, $entryid);
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('qcformbuilder_forms_submit_post_process', $form, $referrer, $process_id, $entryid);
		// POST PROCESS
		foreach ($form['processors'] as $processor_id => $processor) {
			if (isset($form_processors[$processor['type']])) {
				// has processor
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('qcformbuilder_forms_submit_post_process_end', $form, $referrer, $process_id, $entryid);
		 ...
		 * @param int|false $entryid Current entry ID or false if not set or being saved.
		 */
		do_action('qcformbuilder_forms_submit_complete', $form, $referrer, $process_id, $entryid);

		// redirect back or to result page
		$referrer['query']['wfb_su'] = $form_instance_number;

		// last entry id - new only
		if (empty($transdata['edit'])) {
			$wfb_id = self::do_magic_tags('{entry_id}');
			if (!empty($wfb_id)) {
				$referrer['query']['wfb_id'] = self::do_magic_tags('{entry_id}');
			}
		}
		...
		// filter refer
		$referrer = apply_filters('qcformbuilder_forms_submit_redirect', $referrer, $form, $process_id);
		$referrer = apply_filters('qcformbuilder_forms_submit_redirect_complete', $referrer, $form, $process_id);

		// kill transient data
		Qcformbuilder_Forms_Transient::delete_transient($process_id);
	...
	/**
	 * Makes Qcformbuilder Forms load the preview
	 */
	static public function wfb_init_preview()
	{
		if (!isset($_GET, $_GET[Qcformbuilder_Forms_Admin::PREVIEW_KEY])) {
			return;
		}
		...
		$preview_id = trim($_GET[Qcformbuilder_Forms_Admin::PREVIEW_KEY]);
		if (!empty($preview_id)) {
			$form = Qcformbuilder_Forms_Forms::get_form($preview_id);
			...
			if (!empty($userid)) {
				...
				if (empty($post) || $post->post_title !== 'Qcformbuilder Forms Preview') {
					$temp_page = get_page_by_title('Qcformbuilder Forms Preview');
					if (empty($temp_page)) {
						// create page
						$post = array(
							'post_content' => '',
							'post_name' => 'qcformbuilder_forms_preview',
							'post_title' => 'Qcformbuilder Forms Preview',
							'post_status' => 'draft',
							'post_type' => 'page',
							'ping_status' => 'closed',
						...
						$url = add_query_arg(array(
							'page_id' => $page_id,
							'preview' => true,
							Qcformbuilder_Forms_Admin::PREVIEW_KEY => $preview_id

						), home_url());

						if (isset($_GET[Qcformbuilder_Forms_Admin::REVISION_KEY])) {
							$url = add_query_arg(Qcformbuilder_Forms_Admin::REVISION_KEY,
								absint($_GET[Qcformbuilder_Forms_Admin::REVISION_KEY]), $url);
						}
				...
				if (isset($_GET[Qcformbuilder_Forms_Admin::REVISION_KEY])) {
					$post->post_content = '[qcformbuilder_form id="' . $_GET[Qcformbuilder_Forms_Admin::PREVIEW_KEY] . ' revision="' . absint($_GET[Qcformbuilder_Forms_Admin::REVISION_KEY]) . '"]';
				} else {
					$post->post_content = '[qcformbuilder_form id="' . $_GET[Qcformbuilder_Forms_Admin::PREVIEW_KEY] . '"]';
				}
			}
		}
	...
	public function api_handler()
	{
		...
		if (isset($wp_query->query_vars[ 'wfb_api' ])) {
			$form_id = qcformbuilder_forms_very_safe_string($wp_query->query_vars[ 'wfb_api' ]);
		} elseif (isset($_GET, $_GET[ 'wfb-api' ])) {
			$form_id = qcformbuilder_forms_very_safe_string($_GET[ 'wfb-api' ]);
		} else {
			$form_id = null;
		}
		...
		if (!$form_id) {
			...
			if (!empty($_REQUEST[ 'wfb_instance' ])) {
				$atts[ 'instance' ] = $_REQUEST[ 'wfb_instance' ];
			}

			//start buffering
			qcformbuilder_forms_start_buffer();
			...
			if (!empty($form[ 'ID' ])) {
				...
				if ($form[ 'ID' ] === $form_id) {
					/*
					$entry_id = !empty($wp_query->query_vars[ 'wfb_entry' ]) ? $wp_query->query_vars[ 'wfb_entry' ]
						: isset($_GET, $_GET[ 'entry' ]) && absint($_GET[ 'entry' ]) ? $_GET[ 'entry' ] : null;
					*/
					if( !empty($wp_query->query_vars[ 'wfb_entry' ]) ){
						$entry_id = $wp_query->query_vars[ 'wfb_entry' ];
					}else{
						if( isset($_GET, $_GET[ 'entry' ]) && absint($_GET[ 'entry' ]) ){
							$entry_id = $_GET[ 'entry' ];
						}else{
							$entry_id =null;
						}
					...
					if ($_SERVER[ 'REQUEST_METHOD' ] === 'POST') {
						...
						if (!empty($_POST[ 'control' ])) {
							...
							$transdata = Qcformbuilder_Forms_Transient::get_transient($transient_name);
							if (false === $transdata) {
								$transdata = [];
							}
							if (!empty($_FILES) && !empty($_POST[ 'field' ])) {
								$field = Qcformbuilder_Forms_Field_Util::get_field($form[ 'fields' ][ $_POST[ 'field' ] ],
									$form, true);
								$data = wfb_handle_file_upload(true, $field, $form);
								if (is_wp_error($data)) {
									qcformbuilder_forms_send_json($data->get_error_message(), true);
								}
								$transdata[] = $data;
								Qcformbuilder_Forms_Transient::set_transient($transient_name, $transdata, DAY_IN_SECONDS);
								qcformbuilder_forms_send_json(['success' => true]);
							}
						}

						$_POST[ '_wp_http_referer_true' ] = 'api';
						$_POST[ '_wfb_frm_id' ] = $_POST[ 'cfajax' ] = $form_id;
	...
	static function get_submit_url($form_id = '')
	{
		...
		if (is_multisite() && get_blog_option(null, 'permalink_structure') || get_option('permalink_structure')) {
			$url = get_home_url();
			$url .= '/wfb-api/' . ltrim($form_id, '/');
		} else {
			$url = trailingslashit(get_home_url());
			$url = add_query_arg('wfb_api', $form_id, $url);
		}
		if (is_ssl()) {
		...
		/**
		 * Filter the Qcformbuilder Forms APU url
		 *
		 * @since 1.3.2
		 *
		 ...
		 */
		return apply_filters('qcformbuilder_forms_submission_url', $url, $form_id);

	}

	/**
	 * Makes Qcformbuilder Forms go in front-end!
	 */
	static public function wfb_init_system()
	{

		if (!empty($_GET['wfb_tp'])) {

			// process a transient stored entry
			$data = Qcformbuilder_Forms_Transient::get_transient($_GET['wfb_tp']);
			if (!empty($data) && $data['transient'] === $_GET['wfb_tp'] && isset($data['data'])) {
				// create post values
				$_POST = array_merge($_POST, $data['data']);
				// set transient id
				$_POST['_wfb_frm_tr'] = $data['transient'];
			}
		}
	...
	static public function get_entry($entry_id, $form)
	{
		if (is_string($form)) {
			...
			if (!is_array($form) || !isset($form['ID']) || $form['ID'] !== $form_id) {
				return new WP_Error('fail', esc_html__('Invalid form ID', 'qcformbuilder-forms'));
			}
		}
		...
		//See: https://github.com/QcformbuilderWP/Qcformbuilder-Forms/issues/1494
		...
		foreach ($entry as $field_id => $field_value) {
			...
			// set view
			$field_view = apply_filters('qcformbuilder_forms_view_field_' . $field['type'], $field_value, $field, $form);
		...
		if (!empty($form['variables']['types'])) {
			foreach ($form['variables']['types'] as $var_key => $var_type) {
				if ($var_type == 'entryitem') {
					$var_val = Qcformbuilder_Forms::do_magic_tags($form['variables']['values'][$var_key], $entry_id);
		 ...
		 * @param array $form Form config
		 */
		$data['user'] = apply_filters('qcformbuilder_forms_get_entry_user', $data['user'], $entry_id, $form);
		 ...
		 * @param array $form Form config
		 */
		$data = apply_filters('qcformbuilder_forms_get_entry', $data, $entry_id, $form);
	...
	{
		_deprecated_function(__METHOD__, '1.5.0.7', 'Qcformbuilder_Forms_Render_Modals::render_modal_form');
		return Qcformbuilder_Forms_Render_Modals::modal_form($atts, $content);
	}
	...
	{
		_deprecated_function(__METHOD__, '1.5.0.7', 'Qcformbuilder_Forms_Render_Modals::render_footer_modals');
		Qcformbuilder_Forms_Render_Modals::render_footer_modals();
	}
	...
	static public function render_field($field, $form = null, $entry_data = array(), $field_errors = array())
	{
        ...
        $field_id_attr = Qcformbuilder_Forms_Field_Util::get_base_id($field, $current_form_count, $form);
        if( Qcformbuilder_Forms_Field_Util::is_cf2_field_type($type)){
            ...
            $renderer = new \qcformbuilderwp\qcformbuilderforms\cf2\Fields\RenderField($form_id_attribute,
                array_merge( $field, [
		...
		$field_structure = apply_filters('qcformbuilder_forms_render_field_structure', $field_structure, $form);
		$field_structure = apply_filters('qcformbuilder_forms_render_field_structure_type-' . $field['type'],
			$field_structure, $form);
		$field_structure = apply_filters('qcformbuilder_forms_render_field_structure_slug-' . $field['slug'],
			$field_structure, $form);
		 ...
		 */
		$field_file = apply_filters('qcformbuilder_forms_render_field_file', $field_file, $field['type'], $field['ID'],
			$field_file, $field_structure, $form);

		ob_start();
		include $field_file;
		$field_html = apply_filters('qcformbuilder_forms_render_field', ob_get_clean(), $form);
		$field_html = apply_filters('qcformbuilder_forms_render_field_type-' . $field['type'], $field_html, $form);
		$field_html = apply_filters('qcformbuilder_forms_render_field_slug-' . $field['slug'], $field_html, $form);
	...
	static public function render_form($atts, $entry_id = null, $shortcode = null)
	{
		...
		if (is_string($atts)) {
			...
			$form = Qcformbuilder_Forms_Forms::get_form($atts);
			$atts = array();

		} elseif (is_array($atts) && Qcformbuilder_Forms_Forms::is_revision($atts)) {
			$form = $atts;
		...
		if (!empty($form['form_draft'])) {
			if (!isset($_GET['wfb_preview']) || $_GET['wfb_preview'] != $form['ID']) {
				if (isset($_POST['action']) && $_POST['action'] == 'wfb_get_form_preview') {
					echo '<p style="color: #cf0000;">' . esc_html__('Form is currently not active.', 'qcformbuilder-forms') . '</p>';
				} else {
					return;
				}

			} else {
				echo '<div class="qcformbuilder-grid"><p class="alert alert-error alert-danger">' . esc_html__('Form is currently not active.', 'qcformbuilder-forms') . '</p></div>';
			}
		}

		Qcformbuilder_Forms_Render_Assets::optional_style_includes();
		 ...
		 * @param array $form Form config.
		 */
		$form = apply_filters('qcformbuilder_forms_render_get_form', $form);
		 ...
		 * @param array $form Form config.
		 */
		$entry_id = apply_filters('qcformbuilder_forms_render_entry_id', $entry_id, $form);
		 ...
		 */
		$html = apply_filters('qcformbuilder_forms_pre_render_form', null, $entry_id, $form, $atts);
		if (is_string($html)) {
			return $html;
		...
		do_action('qcformbuilder_forms_render_start', $form);
		...
		if (!empty($form['form_ajax'])) {
			add_filter('qcformbuilder_forms_render_form_attributes', 'wfb_ajax_setatts', 10, 2);
		 ...
		 * @param array $config Form config
		 */
		$form_attributes = apply_filters('qcformbuilder_forms_render_form_attributes', $form_attributes, $form);


		include_once WFBCORE_PATH . "classes/qcformbuilder-grid.php";
		 ...
		 */
		$gridsize = apply_filters('qcformbuilder_forms_render_set_grid_size', $gridsize);
		...
		// filter settings
		$grid_settings = apply_filters('qcformbuilder_forms_render_grid_settings', $grid_settings, $form);

		$form['grid_object'] = new Qcformbuilder_Form_Grid($grid_settings);
		...
		if (!empty($entry_id)) {
			...
			if (!empty($_GET['wfb_et'])) {
				// build token
				$validated = Qcformbuilder_Forms_Entry_Token::verify_token(trim($_GET['wfb_et']), $entry_id, $form['ID']);
				if (is_wp_error($validated)) {
					$notices['error']['note'] = __('Permission denied or entry does not exist.', 'qcformbuilder-forms');
				} else {
					$edit_token = Qcformbuilder_Forms_Entry_Token::create_entry_token($entry_id, $form);
				}
		...
		// check for prev post
		$prev_data = apply_filters('qcformbuilder_forms_render_pre_get_entry', array(), $form, $entry_id);
		...
		if (!empty($entry_id)) {
			...
			$prev_data = apply_filters('qcformbuilder_forms_render_get_entry', $prev_data, $form, $entry_id);
		}


		if (!empty($_GET['wfb_er'])) {
			$prev_post = Qcformbuilder_Forms_Transient::get_transient($_GET['wfb_er']);
			if (!empty($prev_post['transient'])) {

				if ($prev_post['transient'] === $_GET['wfb_er']) {
			...
			// filter transient
			$prev_post = apply_filters('qcformbuilder_forms_render_get_transient', $prev_post, $form);

		}
		if (!empty($_GET['wfb_su']) && $current_form_count == $_GET['wfb_su']) {
		...
		if (!empty($page_breaks)) {
			$currentpage = 1;
			if (isset($_GET['wfb_pg']) && !isset($prev_post['page'])) {
				$currentpage = (int)$_GET['wfb_pg'];
			...
			$form['grid_object']->before('<div id="form_page_' . $current_form_count . '_pg_1" data-formpage="1" class="qcformbuilder-form-page" style="display:' . $display . ';" role="region" aria-labelledby="breadcrumb_' . $current_form_count . '_pg_1" aria-hidden="' . $hidden . '">',
				1);
			...
			foreach ($page_breaks as $page => $break) {
				...
				if ($break + 1 <= $total_rows) {
					...
					$form['grid_object']->before('<div id="form_page_' . $current_form_count . '_pg_' . ($page + 2) . '" data-formpage="' . ($page + 2) . '" role="region" aria-labelledby="breadcrumb_' . $current_form_count . '_pg_' . ($page + 2) . '" aria-hidden="' . $hidden . '" class="qcformbuilder-form-page" style="display:' . $display . ';">',
						$break + 1);
				}
			}
		...
		if (!empty($form['fields'])) {
			// prepare fields
			foreach ($form['fields'] as $field_id => $field) {
				$field = apply_filters('qcformbuilder_forms_render_get_field', $field, $form);
				$field = apply_filters('qcformbuilder_forms_render_get_field_type-' . $field['type'], $field, $form);
				$field = apply_filters('qcformbuilder_forms_render_get_field_slug-' . $field['slug'], $field, $form);
				$form['fields'][$field_id] = $field;
			}
		}
			...
			foreach ($form['layout_grid']['fields'] as $field_base_id => $location) {
				...
				if (isset($form['fields'][$field_base_id])) {
					...
					if (empty($field) || !isset($field_types[$field['type']]['file']) || !file_exists($field_types[$field['type']]['file'])) {
                        if (!Qcformbuilder_Forms_Field_Util::is_cf2_field_type($field['type'])) {
                            continue;
                        }
                    }
					...
					Qcformbuilder_Forms_Render_Assets::enqueue_field_scripts($field_types, $field);
					...
					if (!empty($field['conditions']['group']) && !empty($field['conditions']['type'])) {
                        $is_cf2_field = Qcformbuilder_Forms_Field_Util::is_cf2_field_type( Qcformbuilder_Forms_Field_Util::get_type( $field, $form ) );
						...
						// wrapp it up
						$field_html = '<span class="qcformbuilder-forms-conditional-field" role="region" aria-live="polite" id="conditional_' . $field_base_id . '" data-field-id="' . $field_base_id . '">' . $field_html . '</span>';
					}
		...
		// form object strings
		wp_localize_script('wfb-dynamic', $form['ID'] . '_' . $current_form_count, $form_field_strings);

		// do grid
		$form['grid_object'] = apply_filters('qcformbuilder_forms_render_grid_structure', $form['grid_object'], $form);
		// wrapper classes
		 ...
		 * @param array $config Form config
		 */
		$form_wrapper_classes = apply_filters('qcformbuilder_forms_render_form_wrapper_classes', $form_wrapper_classes,
			$form);

		$form_wrap_id = Qcformbuilder_Forms_Render_Util::form_id_attr($current_form_count);
		$out = sprintf('<div class="%s" id="%s" data-wfb-ver="%s" data-wfb-form-id="%s">',
			esc_attr(implode(' ', $form_wrapper_classes)), esc_attr($form_wrap_id), esc_attr(WFBCORE_VER),
			esc_attr($form['ID']));
		...
		if (!empty($form['mailer']['enable_mailer']) && !empty($form['debug_mailer'])) {
			$notices['error'] = array(
				'note' => __('WARNING: Form is in Mailer Debug mode. Disable before going live.', 'qcformbuilder-forms')
			);
		}

		$out .= '<div id="qcformbuilder_notices_' . $current_form_count . '" data-spinner="' . admin_url('images/spinner.gif') . '">';
		if (!empty($notices)) {
			// do notices
			// entry id
			if (isset($_GET['wfb_id'])) {
				$notice_entry_id = (int)$_GET['wfb_id'];
			...
			foreach ($notices as $note_type => $notice) {
				if (!empty($notice['note'])) {
					$out .= '<div class=" ' . implode(' ', $note_classes[$note_type]) . '">' . Qcformbuilder_Forms_Sanitize::remove_scripts(self::do_magic_tags($notice['note'], $notice_entry_id) ) . '</div>';
				}
			}
		...
		if ((empty($notices['success']) || empty($form['hide_form'])) && empty($halt_render)) {
			 ...
			 * @param array $config Form config
			 */
			$form_element = apply_filters('qcformbuilder_forms_render_form_element', $form_element, $form);
			 ...
			 * @param array $config Form config
			 */
			$form_classes = apply_filters('qcformbuilder_forms_render_form_classes', $form_classes, $form);
			...
			$out .= Qcformbuilder_Forms_Render_Nonce::nonce_field($form['ID']);
			$out .= sprintf( '<div id="%s"></div>', esc_attr( "cf2-$form_id_attribute") );
			$out .= "<input type=\"hidden\" name=\"_wfb_frm_id\" value=\"" . $form['ID'] . "\">\r\n";
			$out .= "<input type=\"hidden\" name=\"_wfb_frm_ct\" value=\"" . $current_form_count . "\">\r\n";
			if (!empty($form['form_ajax'])) {
				$out .= "<input type=\"hidden\" name=\"cfajax\" value=\"" . $form['ID'] . "\">\r\n";
			}
			if (is_object($post)) {
				$out .= "<input type=\"hidden\" name=\"_wfb_cr_pst\" value=\"" . $post->ID . "\">\r\n";
			}

			// user transient for continuation
			if (!empty($prev_post['transient'])) {
				$out .= "<input type=\"hidden\" name=\"_wfb_frm_tr\" value=\"" . $prev_post['transient'] . "\">\r\n";
			}
			// is edit?
			if (!empty($entry_id)) {
				$out .= "<input type=\"hidden\" name=\"_wfb_frm_edt\" value=\"" . $entry_id . "\">\r\n";
			}

			// is edit via token?
			if (!empty($edit_token)) {
				$out .= "<input type=\"hidden\" name=\"_wfb_frm_edt_tkn\" value=\"" . $edit_token . "\">\r\n";
			}

			//setup fieldjs
			Qcformbuilder_Forms_Field_Localizer::add_form($form, Qcformbuilder_Forms_Render_Util::get_current_form_count());
			...
			if (!empty($form['auto_progress']) && count($form['page_names']) > 1) {
				...
				$out .= "<span class=\"qcformbuilder-grid\"><ol class=\"breadcrumb\" data-form=\"qcformbuilder_form_" . $current_form_count . "\" id=\"qcformbuilder-forms-breadcrumb_" . $current_form_count . "\">\r\n";
				$current_page = 1;
				if (!empty($_GET['wfb_pg'])) {
					$current_page = $_GET['wfb_pg'];
				}
				foreach ($form['page_names'] as $page_key => $page_name) {
					...
					$qurystr['wfb_pg'] = $page_key + 1;
					$out .= "<li" . $tabclass . "><a aria-controls=\"form_page_" . $current_form_count . "_pg_" . ($page_key + 1) . "\" aria-expanded=\"" . $expanded . "\" id=\"breadcrumb_" . $current_form_count . "_pg_" . ($page_key + 1) . "\" href=\"?" . http_build_query($qurystr) . "\" data-page=\"" . ($page_key + 1) . "\" data-pagenav=\"qcformbuilder_form_" . $current_form_count . "\" title=\"" . sprintf(__('Navigate to %s',
							'qcformbuilder-forms'), $page_name) . "\">" . $page_name . "</a></li>\r\n";
				}
				$out .= "</ol></span>\r\n";
			}
			...
			// sticky sticky honey
			if (Qcformbuilder_Forms_Field_Honey::active($form)) {
				$out .= Qcformbuilder_Forms_Field_Honey::field($form);
			}
		...
		if (!empty($conditions_configs)) {
			...
			$hastags = Qcformbuilder_Forms_Magic_Util::explode_field_magic($conditions_str);
			if (!empty($hastags[1])) {
			...
			$out .= 'if( typeof qcformbuilder_conditionals === "undefined" ){ var qcformbuilder_conditionals = {}; }';
			$out .= "qcformbuilder_conditionals." . $form['ID'] . '_' . $current_form_count . " = " . $conditions_str . ";\r\n";
			$out .= "</script>\r\n";
			...
			Qcformbuilder_Forms_Render_Assets::enqueue_script('conditionals');
		}
		 ...
		 * @param array $config Form config
		 */
		do_action('qcformbuilder_forms_render_end', $form);

		Qcformbuilder_Forms_Render_Assets::enqueue_form_assets();
		 ...
		 * @param array $config Form config
		 */
		return apply_filters('qcformbuilder_forms_render_form', $out, $form);

	}

	/**
	 * Returns the capability to manage Qcformbuilder Forms
	 *
	 * By default, returns "manage_options" can be filtered with "qcformbuilder_forms_manage_cap"
	 *
	 * @since 1.3.1
	 *
	...
	public static function get_manage_cap($context = 'admin', $form = false)
	{
		...
		/**
		 * Change capability for managing Qcformbuilder Forms
		 *
		 * @since 1.3.1
		 *
		 ...
		 */
		return apply_filters('qcformbuilder_forms_manage_cap', 'manage_options', $context, $form);
	 ...
	 * @param string $shortcode Shortcode type qcformbuilder_form|qcformbuilder_forms_modal
	 *
	 * @return string|void
	 */
	public static function shortcode_handler($atts, $content, $shortcode)
	{
		...
		if (!empty($atts['revision'])) {
			$revision = Qcformbuilder_Forms_Forms::get_revision($atts['revision']);
		...
		if ($shortcode === 'qcformbuilder_form_modal' || (!empty($atts['modal']) && $atts['modal'])) {
			return Qcformbuilder_Forms_Render_Modals::modal_form($atts, $content);
		}
	...
	/**
	 * Load the Qcformbuilder Forms REST API
	 *
	 * @since 1.4.4
	 *
	...
	public static function init_rest_api()
	{
		...
		self::$api = new Qcformbuilder_Forms_API_Load(Qcformbuilder_Forms_API_Util::api_namespace());
		 ...
		 *
		 * do_action( 'qcformbuilder_forms_rest_api_pre_init', function( $api ){ $api->add_route(...
		 *
		 * @since 1.4.4
		 *
		 * @param Qcformbuilder_Forms_API_Load $api API Load class
		 */
		do_action('qcformbuilder_forms_rest_api_pre_init', self::$api);

		self::$api->add_route(new Qcformbuilder_Forms_API_Entries());
		self::$api->add_route(new Qcformbuilder_Forms_API_Forms());
		self::$api->add_route(new Qcformbuilder_Forms_API_Settings());
		 ...
		 * @since 1.4.4
         *
         * @param Qcformbuilder_Forms_API_Load $api_v2 Qcformbuilder Forms REST API v2
         * @param \qcformbuilderwp\qcformbuilderforms\cf2\RestApi\Register $api_v3 Qcformbuilder Forms REST API v2
		 */
		do_action('qcformbuilder_forms_rest_api_init',
            self::$api,
            (new \qcformbuilderwp\qcformbuilderforms\cf2\RestApi\Register(Qcformbuilder_Forms_API_Util::api_namespace('v3' ) ) )
                ->initEndpoints()
        );
	...
	public static function should_send_mail($form, $transadata = array())
	{
		 ...
		 * @param array $form Form config
		 */
		return apply_filters('qcformbuilder_forms_send_email', $send, $form);
	...
	{
		if (isset($_POST['_wfb_frm_id'])) {
			qcformbuilder_forms_start_buffer();
			if (isset($_POST['_wfb_verify']) && Qcformbuilder_Forms_Render_Nonce::verify_nonce($_POST['_wfb_verify'],
					$_POST['_wfb_frm_id'])) {
				$submission = Qcformbuilder_Forms::process_submission();
				qcformbuilder_forms_send_json($submission);
				exit;
			}
			...
			$form = Qcformbuilder_Forms_Forms::get_form($_POST['_wfb_frm_id']);
			$notices = array();
			$notices['error']['note'] = __('Submission rejected, token invalid', 'qcformbuilder-forms');
			...
			qcformbuilder_forms_send_json($out,true);
			exit;
	...
	public static function format_message($mail)
	{
		...
		return Qcformbuilder_Forms_Email_Filters::format_message($mail);

	}

	/**
	 * Get main instance of Qcformbuilder_Forms_Settings class
	 *
	 * @since 1.5.3
	 *
	 * @return Qcformbuilder_Forms_Settings
	 */
	public static function settings()
	{
		if (!self::$settings) {
			self::$settings = new Qcformbuilder_Forms_Settings();
			/**
			 * Runs after main instance of Qcformbuilder_Forms_Settings is created
			 *
			 * Access via Qcformbuilder_Forms::settings()
			 *
			 * @since 1.5.3
			 */
			do_action('qcformbuilder_forms_settings_registered');
		}
