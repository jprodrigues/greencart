    ...
    >
		...
		<div id="<?php echo esc_attr($field_id); ?>_settings_pane" class="wrapper-instance-pane">
            ...
            <div class="qcformbuilder-config-group">
				...
				<div class="qcformbuilder-config-field">
					<select class="block-input qcformbuilder-select-field-type" data-field="<?php echo esc_attr($field_id); ?>" id="<?php echo esc_attr($field_id); ?>_type" name="config[fields][<?php echo esc_attr($field_id); ?>][type]" data-type="<?php echo esc_attr( $type ); ?>">
						<?php
			...
			<div class="qcformbuilder-config-group">
				...
				<div class="qcformbuilder-config-field">
					<input type="text" class="block-input field-config field-label required" id="<?php echo esc_attr($field_id); ?>_lable" name="config[fields][<?php echo esc_attr($field_id); ?>][label]" value="<?php echo esc_attr( sanitize_text_field( $label ) ); ?>">
				</div>
			</div>
			...
			<div class="qcformbuilder-config-group">
				...
				<div class="qcformbuilder-config-field">
					<input type="text" class="block-input field-config field-slug required" id="<?php echo esc_attr($field_id); ?>_slug" name="config[fields][<?php echo esc_attr($field_id); ?>][slug]" value="<?php echo esc_attr( $slug ); ?>">
				</div>
			</div>
...
function field_line_template($id = '{{id}}', $label = '{{label}}', $group = '{{group}}'){
	...
	?>
	<li data-field="<?php echo esc_attr( $id ); ?>" class="qcformbuilder-field-line">
		<a href="#<?php echo esc_attr( $id ); ?>">
			<i class="icn-right pull-right"></i>
			<i class="icn-field"></i>
			<?php echo htmlentities( $label ); ?>
		</a>
		<input type="hidden" class="qcformbuilder-config-field-group" value="<?php echo esc_attr( $group ); ?>" name="config[fields][<?php echo esc_attr( $id ); ?>][group]" autocomplete="off">
	</li>
	<?php
					...
					if(!empty($panel['tabs'])){
						...
						foreach($panel['tabs'] as $group_slug=>$tab_setup){
							...
							if($group_slug=='conditions' || $group_slug=='variables' || $group_slug=='processors' || $group_slug=='revisions'){
								echo "<li><p>" . esc_html( $tab_setup['name'] ) . " <span>Pro</span> </p></li>\r\n";
							}else{
								echo "<li". esc_attr( $active )." id=\"tab_". esc_attr( $group_slug )."\"><a href=\"#" . esc_attr( $group_slug ) . "-config-panel\">" . esc_html( $tab_setup['name'] ) . "</a></li>\r\n";
							}
						...
						if(!empty($panel['tabs']['code'])){
							foreach($panel['tabs']['code'] as $code_slug=>$tab_setup){
								...
								echo "<li". esc_attr( $active )."><a href=\"#" . esc_attr( $code_slug ) . "-code-panel\" data-editor=\"" . esc_attr( $code_slug ) . "-editor\">" . esc_html( $tab_setup['name'] ) . "</a></li>\r\n";
							}
						}
...
foreach($panel_extensions as $panel){
	...
	foreach($panel['tabs'] as $panel_slug=>$tab_setup){
		...
		echo "<div id=\"" . esc_attr( $panel_slug ) . "-config-panel\" class=\"qcformbuilder-editor-body qcformbuilder-config-editor-panel " . ( !empty($tab_setup['side_panel']) ? "qcformbuilder-config-has-side" : "" ) . "\"".$active.">\r\n";
			if( !empty($tab_setup['side_panel']) ){
				echo "<div id=\"" . esc_attr( $panel_slug ) . "-config-panel-main\" class=\"qcformbuilder-config-editor-main-panel\">\r\n";
			}
			echo '<h3>'. esc_html( $tab_setup['label'] );
				...
				if( !empty( $tab_setup['repeat'] ) ){
					...
					echo " <a href=\"#" . esc_attr( $panel_slug ) . "_tag\" class=\"add-new-h2 qcformbuilder-add-group\" data-group=\"" . esc_attr( $panel_slug ) . "\">" . esc_html__( 'Add New', 'qcformbuilder-forms' ) . "</a>\r\n";
				}
				// ADD ACTIONS
			...
			if(!empty($tab_setup['fields'])){
				...
				for($group_index = 0; $group_index < $depth; $group_index++){
					...
					if( !empty( $tab_setup['repeat'] ) ){
						echo "<a href=\"#remove_" . esc_attr( $panel_slug ) . "\" class=\"qcformbuilder-config-group-remove\">" . esc_html__( 'Remove', 'qcformbuilder-forms' ) . "</a>\r\n";
						echo "</div>\r\n";
					}
				}
			...
			if(!empty($tab_setup['side_panel'])){
				echo "</div>\r\n";
				echo "<div id=\"" . esc_attr( $panel_slug ) . "-config-panel-side\" class=\"qcformbuilder-config-editor-side-panel\">\r\n";
